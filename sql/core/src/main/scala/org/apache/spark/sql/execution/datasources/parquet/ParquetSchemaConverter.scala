...
class SparkParquetSchemaChecker(
   ...
   * @return fieldNames that cannot be converted.
   */
  def checkType(sparkType: DataType, parquetType: Type): Seq[String] = {
    ...
    if (parquetType.isPrimitive) {
      ...
      if (sparkType.isInstanceOf[ArrayType]) {
        ...
        checkType(arrayType.elementType, parquetType)
      } else {
